...
returns audio files from s3*/
const PlayMeditationApiHandler = {
    canHandle(handlerInput) {
        return util.isApiRequest(handlerInput, 'PlayMeditation');
    },
    handle(handlerInput) {
        console.log("Api Request [PlayMeditation]: ", JSON.stringify(handlerInput.requestEnvelope.request, null, 2));
        // First get today's speech and some audio.
        const speechUrl = util.getS3PreSignedUrl("Media/GvHL2xVq-em2-2.mp3");
        const audioUrl = util.getS3PreSignedUrl("Media/gVSM6X9Q-meditationmusic1.mp3");
        // Store the listened state of today's meditation.
        const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
        sessionAttributes.wasMeditationPlayed = true;

        let response = {
            apiResponse: {
                response_audio : audioUrl,
                response_speech: speechUrl
            }
        };
        console.log("Api Response [PlayMeditation]: ", JSON.stringify(response, null, 2));
        return response;
    }
}
/**
